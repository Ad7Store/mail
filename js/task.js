...
function calculateTotalEarnings(friends) {
    return friends.reduce((total, friend) => {
        if (friend.status === 'verified') {
            // Calculate earnings based on when the friend was verified
        ...
        html += `
            <div class="friend-item ${statusClass}">
                <div class="friend-header">
                    <div class="friend-name">${friend.name || 'Unknown Friend'}</div>
...
document.getElementById('add-friend-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        userData.activities.push({
            ...
            message: `Added friend: ${friendName}`,
        ...
        const allFriendsKey = 'kidwallet_all_friends';
        const allFriendsStr = localStorage.getItem(allFriendsKey);
        let allFriends = allFriendsStr ? JSON.parse(allFriendsStr) : [];
        allFriends.push(newFriend);
        localStorage.setItem(allFriendsKey, JSON.stringify(allFriends));
        
        // Show success message
        showMessage('friend-message', `✅ Friend "${friendName}" added successfully! It will be verified within 72 hours.`, 'success');
